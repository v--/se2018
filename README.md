...
Използва се чист `pdf(la)tex`. В `git` историята може да бъде намерена и настройка за `luatex` с `unicode-math` със `STIX 2` шрифтове, но се оказва, че това забавя компилацията на документа около два пъти. `xetex` не се справя много по-бързо.
