...
\end{proposition}
\begin{proof}
  Нека \( a \) и \( b \) са граници на редицата \( \{ a_k \} \). Ако \( a \) и \( b \) са различни, те непременно имат поне една двойка непресичащи се околности според \cref{thm:real_numbers_are_hausdorff}.
...
\begin{minipage}{0.45\textwidth}
  \begin{theorem}[Геометрична интерпретация на производна]
    ...
    За някое достатъчно малко по абсолютна стойност число \( h \in \R \) разглеждаме секущата \( d_h \) през точките с координати \( (a, f(a)) \) и \( (a+h, f(a+h)) \) (\cref{fig:secant}). При намаляващо \( h \), декартовите уравнения на тези секущи при се приближават към декартово уравнение на допирателна права \( d \) към \( \Gamma_f \) в точката \( a \).
