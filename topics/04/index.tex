...
\subsection{Околности}

От отворените множества ще се ограничим да разглеждаме само крайни отворени интервали, за да не се вдаваме в обща топология.

\begin{definition}
  Ще казваме, че отвореният интервал \( U \) е \textbf{околност} на \( a \), ако \( a \in U \).
\end{definition}
...
\begin{proof}
  ...
  Обратно, ако е зададена околност \( (\alpha, \beta) \) на \( a \), взимаме индекс \( k_0 \), така че \( \Abs{a - a_k} < \beta - \alpha \) при \( k \geq k_0 \), и получаваме, че единствено краен брой членове на редицата с индекси, по-малки от \( k_0 \), потенциално се съдържат извън \( (\alpha, \beta) \).
\end{proof}
