...
\begin{definition}
  \mbox{}
  \begin{enumerate}
    \item Линейният оператор \( L: V_n \to V_n \) наричаме \underLine{симетричен} или \underLine{самоспрегнат}, ако за произволен вектор \( x \in V_n \) е изпълнено равенството
    \begin{align*}
      \Prod x {L(x)} = \Prod {L(x)} x.
    \end{align*}

    \item Квадратната матрица \( A \in \R^{n \times n} \) наричаме \underLine{симетрична}, ако \( A = A^T \).
  \end{enumerate}
\end{definition}
...
\begin{exercise}[\cite{SEAutumn2011}]
