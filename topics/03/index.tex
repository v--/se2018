...
\begin{definition}
  Казваме, че един полином $d \in F[x]$ е \underline{най-голям общ делител} (НОД) на $p \in F[x]$ и $q \in F[x]$, ако $d$ дели $p$ и $q$ и ако всеки общ делител на $p$ и $q$ дели $d$. Ако не е казано иначе, за определеност взимаме $d$ да бъде унитарен и пишем $d = \GCD(p, q)$.
...
\begin{proof}
  ...
  \end{displaymath}
  следователно $g$ дели $p_1$. Със същото разсъждение и с индукция по $i = 1, \ldots, m$ стигаме до извода, че $g$ дели $p_i$, в частност $g$ дели $p_m$. Следователно $p_m = \GCD(p, q)$.
\end{proof}
