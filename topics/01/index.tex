...
\begin{proposition}
  Нека координатната система \( K = Oxy \) е ортонормирана и правата \( l \) има спрямо \( K \) декартово уравнение \( l: y = kx + m \). Дефинираме лъча \( r\ray : y = kx + m \geq 0 \), лежащ върху \( l \). Тогава \( \tan \sphericalangle(r\ray, Ox\ray) = k \).
\end{proposition}
\begin{proof}
  ...
  \begin{equation*}
     \tan \sphericalangle(r\ray, Ox\ray) = \tan \sphericalangle(u, Ox\ray) = k.
  \end{equation*}

  Ако \( k < 0 \), то \( u \updownarrows r\ray \) и
  \begin{equation*}
    \tan \sphericalangle(r\ray, Ox\ray) = \tan(\pi - \sphericalangle(-u, Ox\ray)) = -\tan \sphericalangle(-u, Ox\ray) = -(-k) = k.
  \end{equation*}
\end{proof}
...
\begin{definition}
  ...
  \begin{minipage}{0.45\textwidth}
    Нека сега \( P_0 x\ray \) е лъчът с начало \( P_0 \), успореден на \( Ox\ray \). Ако означим с \( \varphi \) ориентирания ъгъл между лъчите \( P_0x\ray \) и \( P_0 P\ray \), от определенията за \( \sin \) и \( \cos \) за координатите на вектора \( \vect{P_0 P} \) получаваме
    \begin{equation*}
      \begin{cases}
        x - x_0 = r \cos \varphi \\
        y - y_0 = r \sin \varphi
...
\begin{solution}
  ...
  Тогава векторът \( \vect{HG} \) има координати \( \vect{HG}(2 + 2\lambda_G, 5 + 4\lambda_G + 5\mu_H, 3 + \lambda_G - 3\mu_H) \).

  \begin{enumerate}[label=\alph*)]
    \item За да принадлежи точката \( A \) на трансверзалата \( t \), искаме векторите \( \vect{AG}(2\lambda_G, 3 + 4\lambda_G, 3 + \lambda_G) \) и \( \vect{AH}(-2, -2 - 5\mu_H, 3\mu_H) \) да бъдат колинеарни, т.е. \( \exists k \in \BbbR \setminus \set{ 0 }: \vect{AG} = k\vect{AH} \). Разписваме това уравнение покоординатно:
    \begin{align*}
      &\begin{cases}
